use engine::component::{Component, RenderContext, SetupContext, UpdateContext};
use engine::impl_component_any;
use engine::types::{encode_address, Address, Vec2};
    ...
    fn generate_chunk(&mut self, chunk_x: i32, chunk_y: i32) {
        ...
        for ty in 0..CHUNK_SIZE {
            for tx in 0..CHUNK_SIZE {
                ...
                let mut noise = self.normalized_noise(x as f64 / 50.0, self.seed);
                noise += self.normalized_noise((x as f64 / 50.0).sqrt(), self.seed) / 2.0;
                noise += self.normalized_noise((x as f64 / 50.0).powf(0.25), self.seed) / 4.0;
